...
def reverse_string(str)
   return "" if str == ""
    new_str = str.split("")
    tem = new_str.pop.to_s
    tem + reverse_string(new_str.join(""))
end
...
#    p data
#    p "-------------------------"
#    rvalue =  new_arr += flatten(data)
#    p rvalue
#    rvalue
    if data.is_a?(Array)
        data.inject([]) do |result, el|
            if el.is_a?(Array)
                result + flatten(el)
            else    
                result + [el]
